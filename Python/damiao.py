...
if __name__ == "__main__":
    try:
        ...
        init_data1.append(DmActData(
                    motorType=DM_Motor_Type.DM4340_48V,  # 或者具体类型，如 DM_Motor_Type.DM4310
        ...
        init_data1.append(DmActData(
                    ...
                    mst_id=mstid4))
        init_data1.append(DmActData(
                    motorType=DM_Motor_Type.DM4310_48V,  # 或者具体类型，如 DM_Motor_Type.DM4310
                    mode=Control_Mode.MIT_MODE,        # 如 Control_Mode.MIT_MODE
                    can_id=canid5,
                    mst_id=mstid5))
        # init_data1.append(DmActData(
        ...
        prev_vel = 0
        with Motor_Control(1000000, 1000000,"800B0C9530F86497E95D4B0541EF5A30",init_data1) as control:
        #control=Motor_Control(1000000, 5000000,"14AA044B241402B10DDBDAFE448040BB",init_data1) 
            can_ids = [canid1, canid2, canid3, canid4, canid5]
            # hold per-joint positions before converting to ndarray
            pos = [0.0] * 5
            sum_interval = 0.0
            last_pos = 0.0
            cyclecnt = 1
            i = 0
            #np.arrays=np.append(np.arrays,0.0)
            #np.savetxt("data.txt",np.arrays)
            flag = True
            torque_incrementor = 5000 * 20 * 0.6 #temporary variable for friciton testing
            while running.is_set():
                    desired_duration = 0.001  # 秒
                    current_time = time.perf_counter()
                    # kp: float, kd: float, q: float, dq: float, tau: float)
                        #setpoint = (torque_incrementor // 5000)/20
                        #torque_incrementor += 1
                        #setpoint = setpoint % 2
                    for key, cid in enumerate(can_ids):
                        pos[key] = control.getMotor(cid).Get_Position()
                    qpos = np.asarray(pos)
                    joint_torques = ik(qpos)
                    control.control_mit(control.getMotor(canid1), 10, 0, 0.0, 0.0, joint_torques[0]) #def control_mit(self, DM_Motor, kp: float, kd: float, q: float, dq: float, tau: float)
                    control.control_mit(control.getMotor(canid2), 10, 0, 0.0, 0.0, joint_torques[1])
                    control.control_mit(control.getMotor(canid3), 10, 0, 0.0, 0.0, joint_torques[2])
                    control.control_mit(control.getMotor(canid4), 10, 0, 0.0, 0.0, joint_torques[3])
                    control.control_mit(control.getMotor(canid5), 2, 0, 0.0, 0.0, joint_torques[4])
                    ...
                    print(f"POS: {np.array2string(qpos, formatter={'float_kind':lambda x: '%.3f' % x})} | TAU: {np.array2string(joint_torques, formatter={'float_kind':lambda x: '%.3f' % x})}", file=sys.stderr)
            ...
            #plt.plot(array_time, array_tau, label='Torque Time Graph')
            #plt.show()
